...
export default class NftContractProvider {
    public static async getContract(address?: string): Promise<ContractType> {
        if (!address) {
            // Check configuration
        ...
        return await ethers.getContractAt(CollectionConfig.contractName, address) as unknown as ContractType;
    }
};
